...
**Branch**: `004-frame-dedup-observability` | **Date**: 2026-07-22 | **Spec**: [spec.md](spec.md)
...
| `vnc_agent/src/vnc_agent/storage/artifact_store.py` | 提供显式用途、实际字节数、可选 private 持久化和原子成组写入；移除路径字符串安全推断 |
...
Supporting changes：`domain/run.py` 保存 frames/telemetry/summary；`api/cli.py` 为 execute 与离线 report 装配共享服务、telemetry 和 locale；`structured_screen.py`、`screen_diff.py`、OCR/template 入口接受已解码数组；`storage/repositories.py` 增加 JSON payload round-trip 验证但不迁移表。
...
2. 使用 `cv2.imdecode(..., IMREAD_UNCHANGED)` 一次解码，规范化为只读、C-contiguous 数组；从 shape/dtype/channel order 得到稳定 `pixel_format`。
3. 对域分隔后的 width/height/pixel_format 与遮罩前像素 bytes 计算 SHA-256，并记录 `pixel_hash` 测量。
...
- 每次成功捕获均创建新的 frame id、step id 和 UTC timestamp，并由 FrameCaptureService 内部唯一 recorder 在返回调用方前追加到 `TestRun.frames`；Pipeline、StabilityEngine、重试和验证调用方不得各自维护不完整的轨迹。
...
- `ArtifactStore` 使用临时文件加原子 replace；安全 bytes 与可选私有 bytes 全部准备成功后才提交。路径用途为显式元数据，不再根据目录名猜测。
- `TestRun.frames` 按 capture sequence 保存观察、等待、重试和验证产生的所有成功逻辑帧；失败尝试进入事件流而不伪造成功 `ScreenFrame`。

### 3. 共享捕获会话与有界缓存

CLI 装配一个 `FrameCaptureService`，同时注入 `ObservationPipeline` 与 `StabilityEngine`。`AgentRuntime` 在 connect 后开启 run/session，在成功 reconnect 后旋转 `vnc_session_id` 并清空 previous/cache；run 完成或 disconnect 时显式 `clear()`。
...
- Planner、Grounder、Verifier 以及带步骤意图/验证问题的回答不进入图片内容缓存。确定性路由比较请求语义和完整上下文身份：任一变化执行所需调用；全部相同且阶段无需新计划时记录 Planner skip；Verifier 对每个操作后观察始终独立执行。
...
- 每次真实 capture 都写逻辑轨迹、调用 `early_exit` 并参与 `stable_frame_count`。
...
`report_builder.py` 不再改写 iteration 路径或调用 `copy_masked_for_report`。它从逻辑帧的显式 safe artifact 引用解析证据，验证路径位于当前 run 安全根、用途正确、文件存在且 mask identity 匹配，然后生成相对 HTML 链接；异常证据显示明确状态，绝不回退引用私有图片。

`report_build` 采用两阶段、无自引用边界：计时阶段完成“安全证据解析 + 基础机器 report dict + 本地化 view-model 草稿组装”，随后立即结束测量并把该 measurement 追加到 TestRun；最后只执行确定性的 measurement 注入与不可变冻结，JSON/HTML renderer 共同消费该唯一冻结视图。最终编码/写盘另记 `report_output`，不属于 `report_build`。
...
展示 view-model 同时提供 `machine_value`、`display_value`、稳定 CSS class/data-marker。Jinja 模板只消费资源键和展示值，不包含状态/错误翻译分支。未知错误使用通用中文说明并完整保留原始 code/detail。Jinja Environment 开启 autoescape；JSON/HTML 继续显式 UTF-8，JSON 保持 `ensure_ascii=False`。

### 8. JSON 向后兼容

`json_report.py` 保持现有顶层键、steps/iterations 结构、英文字段、枚举、类型与语义不变，仅追加：
...
`RunRepository` 继续保存整个 `TestRun` JSON payload，无需表结构迁移；增加 round-trip 测试证明多逻辑帧共享路径和独立时间戳可完整恢复。旧 JSON golden 采用兼容投影逐字段比较，不要求新对象与旧对象完全相等。

### 9. 安全回退

- decode/hash/逐像素比较/cache get/cache put 异常：记录 optimization failure，按唯一帧/cache miss 走完整安全持久化和分析；不得增加 hit、avoided 或 skipped 指标，也不得绕过 `private_persistence_allowed=false`。
- 有遮罩时 decode/遮罩编码失败：fail closed，不允许把原始 bytes 写到安全路径。
- 任一必需物理写入失败：不返回成功 `ScreenFrame`，清理临时/部分文件，交给现有确定性错误/恢复路径。
...
1. 单元测试：一次解码、像素 hash、碰撞、单像素变化、scope/mask/model/config key、容量 3/5、内存释放、故障注入、指标守恒、locale 资源覆盖。
2. 固定截图离线测试：10 相同 + 第 11 单像素、A→B→A 非相邻 miss、不同编码同像素、不同 ROI/遮罩、禁止 private 持久化、`stable_frame_count=3`，并核对观察/等待/重试/验证帧全部进入同一轨迹。
3. 报告测试：中文 HTML 完整快照、UTF-8/autoescape、旧 JSON golden 兼容投影、同一证据路径零副本、安全路径负例。
...
6. 跨场景契约：`generic-form-flow` 与 `generic-icon-menu-flow` 以不同交互路径运行同一 capture→observe→act→verify→report 契约，核心实现不得读取 fixture 名称或内容词汇。
