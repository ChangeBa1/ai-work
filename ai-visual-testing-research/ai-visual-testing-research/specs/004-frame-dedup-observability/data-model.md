...
| `content_hash` | string or null | hash 优化失败时 null |
...
**Relationship**: 一个唯一 `ScreenFrame` 对应一个 safe PhysicalImageRef，并在策略允许时对应第二个 private PhysicalImageRef；多个连续重复 ScreenFrame 只能引用当前策略允许的同一组 PhysicalImageRef。
...
| `content_hash` | string or null | yes | 遮罩前规范化像素 SHA-256；hash 失败为 null |
...
| `physical_image_count` | int | 成功 physical_image_written 事件数 |
...
**Conservation**: `unique_frame_count + duplicate_frame_count == total_capture_count`。正常报告构建 `physical_images_by_purpose.report_copy == 0`。
...
```text
CAPTURED_BYTES
  ├─ decode/hash optimization failure ──> UNIQUE_FALLBACK
  └─ NORMALIZED ──> SCOPE_CHECK ──> HASH_CHECK ──> PIXEL_EQUAL_CHECK
                                             ├─ true  ──> DUPLICATE_REUSE
                                             └─ false ──> UNIQUE_PERSIST

UNIQUE_FALLBACK ──> UNIQUE_PERSIST_SAFE [──> UNIQUE_PERSIST_PRIVATE if allowed]
UNIQUE_PERSIST_SAFE ──> LOGICAL_FRAME_RECORDED ──> ANALYZE/ASSEMBLE ──> RELEASE/ENQUEUE
DUPLICATE_REUSE ──> LOGICAL_FRAME_RECORDED ──> CACHE/ASSEMBLE ──> RELEASE/ENQUEUE

mask/persistence failure ──> CAPTURE_FAILED ──> existing deterministic error/recovery
```

只有 `LOGICAL_FRAME_RECORDED` 状态进入 JSON `frames`；失败尝试通过 measurement/counter/log 保持审计。
