...
- 不同 PNG 编码但相同规范化像素得到相同 content hash；
...
- `stable_frame_count=3` 在第 3 个逻辑采样后稳定，即使后两帧物理去重；
- `early_exit` 每个逻辑采样仍调用。
...
逻辑轨迹必须分别包含 ObservationPipeline 观察、StabilityEngine 等待、重试和操作后验证的成功采集，且全部由同一 capture sequence 排序。

## 4. Fault fallback

```powershell
uv run pytest -q tests/unit/test_capture_fallback.py tests/unit/test_artifact_persistence_failure.py
```

Expected:

- hasher/array compare/cache get/cache put 异常：unique/cache miss + 完整分析；
- optimization failure 不增加 hit、skipped 或 avoided；
- safe/private 必需写入失败：不产生成功 ScreenFrame、不进入 Verifier；
...
5. run 目录不存在由正常报告构建新增的 evidence PNG；
